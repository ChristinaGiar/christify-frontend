...
const SearchResults = ({ query, title, type }) => {
  ...
  const searchInit = async (value, offset = 0) => {
    ...
    const searchOutput = await fetch(`http://localhost:3000/${urlPath}?q=${value}&offset=${offset}&limit=10`, {
      method: 'GET',
    ...
    switch (type) {
      case "albums":
        return results.map((item, index) =>
          <Album key={index} name={item.name} albumID={item.albumID} artists={item.artists} image={item.image?.url} type="album" />)
      case "tracks":
        return results.map((item, index) =>
          <Track key={index} name={item.name} trackID={item.trackID} artists={item.artists} image={item.image?.url} song={item.song} album={item.album} type="track" />)
      default:
        return "";
    }
  }
